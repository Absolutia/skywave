# skywave
Chat program. Or its supposed to be. Doesn't really resemble one right now.
Currently for command line Linux/BSD only, but we have planned a DOS port as well as GUI versions for Windows & Android.
## Requirements
<b>ncurses</b> and <b>pthread</b>, if for some reason your BSD or GNU/Linux distro doesn't come with them.
As for hardware requirements... no idea for certain. We'll have to wait and see, I suppose.
